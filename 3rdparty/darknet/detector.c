...
void detector_loaded_test()
{
	printf("detector loaded!");
}

void detector_set_config_files_absolute_path(detector* det, char* path)
{
	if (path)
		det->config_files_absolute_path = path;
	else
		det->config_files_absolute_path = "";
}

void detector_load_data(detector* det, char* datacfg)
{
	det->options = read_data_cfg(datacfg);
	char* name_list = option_find_str(det->options, "names", "data/names.list");

	char temp[256];
	char* p = det->config_files_absolute_path;
	int i = 0;
	while (*p != '\0')
	{
		temp[i] = *p;
	...
	strcat(temp, name_list);

	det->names = get_labels(temp);
}

void detector_load_model(detector* det, char* cfgfile, char* weightfile, void(*progress)(float))
{
	det->net = parse_network_cfg_custom_with_progress(cfgfile, 1, progress);
	
	if (weightfile) 
	{
		load_weights(&det->net, weightfile);
	}

	fuse_conv_batchnorm(det->net);
}

detect_result detector_detect(detector* det, char* filename, float thresh, float hier_thresh)
{
	...
	if (filename) 
	{
		...
		int letterbox = 0;
		image sized = letterbox_image(im, det->net.w, det->net.h); letterbox = 1;
		...
		result.results = get_network_boxes(&det->net, im.w, im.h, thresh, hier_thresh, 0, 1, &result.result_num, letterbox);
		if (nms) do_nms_sort(result.results, result.result_num, l.classes, nms);
...
{
	free_ptrs(det->names, det->net.layers[det->net.n - 1].classes);
	free_list_contents_kvp(det->options);
	free_list(det->options);
	free_network(det->net);
}

void detect_result_free(detect_result* result)
{
	free_detections(result->results, result->result_num);
}
...
{
	detector det;
	detector_set_config_files_absolute_path(&det, NULL);
	detector_load_data(&det, "data/coco.data");
    detector_load_model(&det, "yolov3.cfg", "yolov3.weights", NULL);
	detect_result r = detector_detect(&det, "dog.jpg", 0.3, 0.5);

	image img = load_image_color("dog.jpg", 0, 0);
	//printf("%d", imgg.h);
	draw_detections_v3(img, r.results, r.result_num, 0.3, det.names, 0, det.net.layers[det.net.n - 1].classes, 1);
	save_image(img, "predictions");
	show_image(img, "predictions");

	free_image(img);

	//detect_result_free(&r);
	//detector_free(&det);
